...
diag_log format ["[DZAI Клиент]: Инициализация: %1 Версия: %2.",DZAI_CLIENT_TYPE,DZAI_CLIENT_VERSION];

call compile preprocessFileLineNumbers "Scripts\DZAIClient\dzai_client_config.sqf";
...
if (DZAIC_deathMessages) then
{
	"DZAI_killMSG" addPublicVariableEventHandler {
		systemChat format ["%1 был убит",(_this select 1)];
		//diag_log format ["[DZAI Клиент]: [ОТЛАДКА] %1 был убит",(_this select 1)];
	};
};
