...
if (!isDedicated) then
{
	...
	timezoneswitch 				= 	3; 			// Оффсет времени для MurderMenu (Сам не шарю конкретику)
	...
	DZE_limitPlots 					= 	1; 			// Лимит установки ЗНАК-ПЛОТА на Игрока, Установите 0 - Отключить. UIDы указанные в DZE_PlotManagementAdmins списке имеют исключение на этот лимит.
	DZE_maintainCurrencyRate 		= 	500; 		// Множитель Валюты для Обслуживания предметов на Базе. НАПРИМЕР: Множитель 100 * 10 предметов = 1000 (Слиток: 1 10oz gold или 1k монет). Cмотри Dayz_code/actions/maintain_area.sqf для примеров.
...
ZSC_CoinForNewPlayer = [true,[10,50000]]; // Выдавать деньги Новым игрокам? Считаются только игроки которые зашли на сервер первый раз. True - Вкл / False - Выкл // Значения 0 - 5: Это От скольки и До скольки денег может быть случайно у Зомби.
...
// Group System
dayz_groupSystem 	= 	true; 		// Включить group system? True - Да / False - Нет.
