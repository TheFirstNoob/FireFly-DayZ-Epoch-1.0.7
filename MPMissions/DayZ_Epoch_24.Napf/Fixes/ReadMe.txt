ВНИМАНИЕ: Принятые исправления были согласованы согласно GitHub DayZ Epoch под 1.0.7.1 версию.
Следующие файлы могут быть удалены, а пути к файлам могут быть установлены по умолчанию после релиза 1.0.7.1:

WARNING: The accepted fixes have been aligned according to the GitHub DayZ Epoch under version 1.0.7.1.
The following files may be removed and file paths may be set to default after the 1.0.7.1 release:

COMPILE: (Стандартный путь/Default Path: \z\addons\dayz_code\compile\...)
- player_attackfsmdelay.sqf
- fn_inAngleSector.sqf
- player_music.sqf
- player_checkStealth.sqf
- fn_sunRise.sqf
- player_gearSet.sqf
- player_unlockVault.sqf
- player_lockVault.sqf
- veh_handleKilled.sqf
- fn_buildWeightedArray.sqf
- fn_inString.sqf
- player_projectileNear.sqf
- fn_selectRandomLocation.sqf
- fnc_getSetPos.sqf
- player_throwObject.sqf 

ACTIONS: (Стандартный путь/Default Path: \z\addons\dayz_code\actions\...)
- changeCode.sqf - Смотри/Check fn_selfAction!!!
- remove.sqf - Смотри/Check fn_selfAction!!!
- player_upgrade.sqf - Смотри/Check fn_selfAction!!!
- player_buildingDowngrade.sqf - Смотри/Check fn_selfAction!!!
- player_emptyContainer.sqf
- modular_build.sqf