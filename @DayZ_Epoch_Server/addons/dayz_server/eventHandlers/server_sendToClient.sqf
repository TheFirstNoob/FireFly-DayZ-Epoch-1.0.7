...
if (_variable in ["Unconscious","DragPlayer","DropPlayer","Legs","RemoveObject","CarryPlayer","LoadWounded","UnloadWounded"]) then
{
	...
	_objPos 			= 	_verifyArray select 0; 	// Может быть объект или Позиция
...
call {
	...
	if (_variable == "Humanity") exitWith
	{
		...
		//diag_log ("[СЕРВЕР]: [server_sendToClient.sqf]: Человечность: " +str(PVCDZ_plr_Humanity));
	};
	...
	if (_variable == "dayzSetDate") exitWith
	{
		...
		//diag_log ("[СЕРВЕР]: [server_sendToClient.sqf]: Время и Дата: " +str (dayz_storeTimeDate));
	};
