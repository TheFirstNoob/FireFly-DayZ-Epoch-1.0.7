...
if (count _medical > 0) then
{
	...
	_playerObj setVariable ["messing", 
										if (count _medical >= 14) then
										{
											(_medical select 13)
										}
										else
	...
	_playerObj setVariable ["blood_testdone",
												if (count _medical >= 15) then
												{
													(_medical select 14)
												}
												else
...
for "_i" from 0 to 12 do {
	_randomKey set [_i, (ceil(random 128)) + 256]; //Latin Extended-A characters не фильтруются в publicvariableval.txt
};
